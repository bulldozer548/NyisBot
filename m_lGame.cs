...
namespace MAIN
{
	...
	class m_lGame : Module
	{
		...
		public m_lGame(Manager manager) : base("lGame", manager)
		{
			...
			var sub = p_manager.GetChatcommand().Add(G.settings["prefix"]+"lgame");
			m_subcommand = sub;
		...
		void Cmd_join(string nick, string message)
		{
			...
			if (count > 2)
				text += $" If you want to start the game, use \"{G.settings["prefix"]}start\"";
			else if (count == 1)
		...
		void Cmd_start(string nick, string message)
		{
			...
			channel.Say("Game started! Player " + game.players[0].nick +
				$" can play the first card using \"{G.settings["prefix"]}add <'main card'> <card nr.> [<card nr.> [<card nr.>]]\"" +
